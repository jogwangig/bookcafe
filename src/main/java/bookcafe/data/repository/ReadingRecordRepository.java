...
public interface ReadingRecordRepository extends JpaRepository<ReadingRecord, Long>{
	...
	@Query("SELECT new bookcafe.data.dto.ReadingRecordDetailDto(rr.id ,rr.cratedAt, rr.content, b.bookInfo.title) FROM ReadingRecord rr "+
		      "LEFT JOIN rr.book b WHERE rr.user.id = :userId")
	Page<ReadingRecordDetailDto> findByUserId(@Param("userId") Long id , Pageable pageable);
	
	@Query("SELECT new bookcafe.data.dto.ReadingRecordDetailDto(rr.id , rr.cratedAt, rr.content, b.bookInfo.title) FROM ReadingRecord rr "+
	      "LEFT JOIN rr.book b WHERE b.id = :bookId")
	Page<ReadingRecordDetailDto> findByBookId(@Param("bookId") Long id , Pageable pageable);
